        ...
        if amountDetecting != 0:
            for (a, b, width, height) in detecting:
                ...
                if amountDetecting2 != 0:
                    ...
                    return False, imaging, x_ball, y_abll,a
    return True, imaging, 0, 0, 0
...
def lets_go_gamepad(x, y, gamepad, ball_size):
    gamepad.reset()
    tmp_trigger =1 - ((ball_size-50)/330)
    if tmp_trigger == 0:
        tmp_trigger = 0.5
...
def main():
    ...
    print('Нажмите пробел как услышите звуковой сигнал подключения геймпада, затем через 5 сек алгоритм запустится, для выхода зажмите ~(ё) на 10-15 сек')
    keyboard.wait('space')
    ...
    while True:
        ...
        x_ball, y_ball, ball_size = 0, 0, 0

        for i in range(1, 5):
            Flag_b_not_seek, imaging, x_ball, y_ball, ball_size = find_ball(i, xml_data, screenshot)
        ...
        # cv2.waitKey(1)
        lets_go_gamepad(x_ball, y_ball, gamepad, ball_size)
